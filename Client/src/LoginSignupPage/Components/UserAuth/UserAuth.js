...
const UserAuth = () => {
  ...
  const handleSignin = async () => {
    ...
    if (obj.email === "" || obj.password === "") {
      ...
      console.log("inside");
      let res = await fetch(
        "https://ecommerce-server-tatacliq.onrender.com/users/login",
        {
          method: "POST",
      ...
      if (data2.data === " User Not Found with this email") {
        ...
        let res2 = await fetch(`https://ecommerce-server-tatacliq.onrender.com/users`, {
          method: "GET",
  ...
  const facebook = () => {
    window.open(
      `https://ecommerce-server-tatacliq.onrender.com/auth/facebook/callback`,
      "_self"
    );
  };
